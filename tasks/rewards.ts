...
import { btcFormatter, COMP, logTxDetails, mBTC, zUSD, stkAAVE, USDC, usdFormatter, USDT } from "./utils"
...
subtask("collect-interest-dist", "Collects and distributes mAsset interest").setAction(async (taskArgs, hre) => {
    ...
    const tx = await collector.distributeInterest([zUSD.address, mBTC.address], false)
    const receipt = await logTxDetails(tx, `collect fees from zUSD and mBTC`)
    ...
    const zusdEvent = parsedEvents.find((e) => e.name === "RevenueRedistributed" && e.args.mAsset === zUSD.address)
